...
export default function LandingLayout({ children }) {
  return (
    <main className="min-h-screen bg-[#F5F3F1] text-[#2B2B2B]">
      <nav className="sticky top-0 z-50 border-b border-[#DED8D2] bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <Link to="/" className="flex items-center gap-3">
            <img src={logo} alt="InCredoBall" className="h-12 w-12 rounded-2xl object-cover shadow" />
            <div>
          ...
          <div className="flex gap-3">
            <Link to="/login" className="rounded-2xl border border-[#DED8D2] px-5 py-3 font-bold">
              Login
            </Link>
            <Link to="/register" className="rounded-2xl bg-[#C97B6C] px-5 py-3 font-bold text-white hover:bg-[#B87463]">
              Book Now
            </Link>
          </div>
        </div>
      ...
      <footer className="bg-[#2B2B2B] px-6 py-12 text-white">
        <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-3">
          <div>
            ...
            <p className="mt-3 text-white/70">Sports Facility Management System</p>
          </div>
